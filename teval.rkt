...
(define (apply-generic-function generic-function arguments)
  (let ((methods (compute-applicable-methods-using-classes
    ...
    (if (null? methods)
        (error "No method found -- APPLY-GENERIC-FUNCTION:" (generic-function-name generic-function))
...
(define (mysort lst proc)
  (list->mlist (sort (mlist->list lst) proc)))
...
(define (method-more-specific? method1 method2)
  ;(display "In method-more-specific?")   ;debug
...
(define (print object)
  (cond ((standard-instance? object)
        ...
        ((class? object)
         (displayln (list 'the 'class (class-name object))))
...
(define initial-procedures
  (list
   ...
   (list 'square '(<number>) (lambda (x) (* x x)))
   (list 'expt '(<number> <number>) expt)
   
   (list 'eq? '(<object> <object>) eq?)
...
(define (tool-print x)
  (tool-apply (tool-eval 'print glb-env)
              (list x)))
...
(define (driver-loop)
  ...
  (let ((input (read)))
    (if (not (eof-object? input))
        (begin
          (with-handlers
              ((exn:fail? (lambda (x) (displayln (exn-message x)))))
            (let ((result (tool-eval input glb-env)))
              (if (not (void? result))
                  (tool-print result))))
          (driver-loop)))))
...
(define (initialize-tool)
  (set! glb-env (make-initial-environment)))
